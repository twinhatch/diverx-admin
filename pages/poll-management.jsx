...
const PollManagement = (props) => {
  ...
  // const [opt, setOpt] = useState( {title:''})
  useEffect(() => {
    getService()
  }, [])
  console.log(data)
  const getService = async () => {
    props.loader(true);
    Api("get", '/api/getallpoll', '', router).then(
      (res) => {
        props.loader(false);
        console.log("res================>", res);
        setServiceList(res)
  ...
  const deleteService = async (id) => {
    props.loader(true);
    Api("delete", `/api/deletepoll/${id}`, '', router).then(
      (res) => {
        props.loader(false);
        console.log("res================>", res);
        getService()
  ...
  const submit = (e) => {
    ...
    console.log(data)
    let method = "post"
    let url = "/api/postpoll"
    if (data._id) {
      url = `/api/updatepoll/${data._id}`
      method = "put"
    }
    ...
    return (
      <div className=' p-4  flex items-center  justify-center'>
        <button className='h-[38px] w-[89px] bg-custom-blue text-custom-darkBlack text-base	font-normal Poppins'
          onClick={() => { setData(row.original), setPopUpservice(true) }}
        >Edit</button>
  ...
  const statussHandler = ({ value, row }) => {
    return (
      <div className="p-4   flex items-center  justify-center">
        <p className="font-semibold text-md text-black w-[200px]  line-clamp-3 truncate text-center">
          {value}
        </p>
      </div>
    );
            ...
            <form className="px-[20px]" onSubmit={submit}>
              ...
              {data?.options?.map((item, ind) => (
                <div className="border-2 border-sky-500 rounded-[15px] overflow-hidden mt-[10px] h-[60px] flex items-center">
                  ...
                  />
                  {data?.options?.length === ind + 1 && <div className="flex justify-end">
                    <button className="h-[40px] w-[100px] bg-[#0D97AE] p-1 rounded-[10px] text-white  text-center mr-[5px]" onClick={() => {
                      setData({ ...data, options: [...data.options, { title: '' }] })
                    }}>
                      Add More
                    </button>
                  </div>}
              ...
              <div className="flex justify-center">
                <button className="h-[50px] w-[130px] bg-[#0D97AE] p-2 rounded-[10px] text-white my-[30px] text-center" type="submit">
