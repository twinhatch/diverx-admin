...
const Database = (props) => {
    ...
    const nameHandler = ({ value }) => {
        return (
            <div className=' p-4  flex flex-col items-center  justify-center'>
                <p className='font-semibold text-md '>{value.fullName}</p>
    ...
    const statussHandler = ({ value, row }) => {
        return (
            <div className='p-4 flex items-center  justify-center'>
                <p className='font-semibold max-w-xs whitespace-break-spaces text-md text-black'>{value}</p>
            </div>
        )
    }
    ...
    const filterData = (event) => {
        ...
        if (event?.target?.value.length > 0) {
            const newdata = mainList.filter(f => f.track_id.includes(event.target.value))
                ...
                <div className='fixed top-0 left-0 w-screen h-screen bg-black/30 flex justify-center items-center z-50'>
                    <div className=' relative w-[300px] md:w-[450px] h-[400px] bg-white border-2 border-custom-blue rounded-md m-auto'>
                        ...
                        <div className=' w-full  p-5 text-xl font-medium text-start'>
                            <p className='pb-1'><span className='font-bold'>Traveller Name</span>{`: ${popupPackage?.user?.fullName}`}</p>
                            <p className='pb-1'><span className='font-bold'>Phone Number</span>{`: ${popupPackage?.user?.phone}`}</p>
                            <p className='pb-1'><span className='font-bold'>From Address</span>{`: ${popupPackage?.fromaddress}`}</p>
                            <p className='pb-1'><span className='font-bold'>To Address</span>{`: ${popupPackage?.toaddress}`}</p>
                            <p className='pb-1'><span className='font-bold'>Last Track</span>{`: ${popupPackage?.track_address}`}</p>
                        </div>
